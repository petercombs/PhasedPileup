...
def get_snps(snpfile):
    ...
    if path.exists("true_hets.tsv"):
        true_hets = {
            tuple(line.strip().split()): True for line in open("true_hets.tsv")
        }
    else:
...
def draw_read(
            ...
            dwg.polygon(
                [
                    ...
                    (
                        x_scale
                        * (
                            min(block_start + read_height / 2, block_end)
                            - x_start_coord
                        ),
                        y_coord + read_height,
                    ),
            ...
            dwg.polygon(
                [
                    ...
                    (
                        x_scale
                        * (
                            max(block_start, block_end - read_height / 2)
                            - x_start_coord
                        ),
                        y_coord + read_height,
                    ),
    ...
    for i in range(slice_start, slice_end):
        ...
        g.add(
            dwg.line(
                (x_scale * (last_stop - x_start_coord), y_coord + 0.5 * read_height),
                (x_scale * (block_start - x_start_coord), y_coord + 0.5 * read_height),
        ...
        g.add(
            dwg.rect(
                (x_scale * (block_start - x_start_coord), y_coord),
                (x_scale * (block_stop - block_start), read_height),
                class_="read{}".format(phase_color),
                id=read.qname,
            )
        )
    ...
    if last_read is not None and read.qname == last_read.qname:
        g.add(
            dwg.line(
                (
                    x_scale * (last_read.reference_end - x_start_coord),
                    y_coord + read_height / 2,
                ),
                (
        ...
        g.add(
            dwg.polygon(
                [
                    ...
                    (
                        x_scale * (read_start_offset - x_start_coord),
                        y_coord + read_height / 2,
                    ),
...
def parse_args():
    ...
    parser.add_argument(
        "--coords", "-c", default=None, help="Format: chrom:10..100 or chrom:10-100"
    )
    ...
    parser.add_argument("--skip-if-deep", "-d", default=False, action="store_true")
    parser.add_argument("--draw-all-snps", "-S", default=False, action="store_true")
    ...
    if args.coords is not None:
        ...
        for row in open(args.gtf_file):
            row = row.split("\t")
            if ('"{}"'.format(args.gene_name) in row[-1]) or (
                "={};".format(args.gene_name) in row[-1]
            ):
                min_coord = min(min_coord, int(row[3]))
                max_coord = max(max_coord, int(row[4]))
                args.chrom = row[0]
                curr_transcript = find_transcript(
                    row[-1], "=" if args.gtf_file.endswith(".gff") else " "
                )
...
def draw_exons(dwg, exons, x_start, y_start):
    ...
    for left, right, id in exons:
        ...
        g.add(
            dwg.line(
                (x_scale * (left - x_start), 0),
                (x_scale * (left - x_start), y_start + read_height * curr_transcript),
            )
        )
        g.add(
            dwg.line(
                (x_scale * (right - x_start), 0),
                (x_scale * (right - x_start), y_start + read_height * curr_transcript),
            )
        )
        g.add(
            dwg.rect(
...
if __name__ == "__main__":
    ...
    for fname in args.samfile:
        for read in Samfile(fname).fetch(gene_chrom, gene_coords[0], gene_coords[1]):
            ...
            for _, pos in read.get_aligned_pairs(matches_only=True):
                if gene_coords[0] <= pos <= gene_coords[1]:
                    break
            else:
                continue
            ...
            if read.qname in unmatched_reads[read.is_read1]:
                ...
                unmatched_reads[read.is_read2][read.qname] = (read, phase)
    ...
    if args.gene_name:
        out_fname = path.join(
            path.dirname(args.outdir or out_fname),
    ...
    for row_num, row in enumerate(phase_neg):
        ...
        for read in row:
            ...
            draw_read(
                ...
                dwg,
                start_coord,
                y_start - row_num * 1.2 * read_height,
                "neg",
    ...
    if args.draw_all_snps:
        ...
        for snp in snps[gene_chrom]:
            if snp < start_coord or snp > end_coord:
                continue
            g.add(
                dwg.line(
